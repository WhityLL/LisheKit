...
class AliPayManager: NSObject{
    ...
    func payAlertController(_ sender: UIViewController,
        ...
        AlipaySDK.defaultService().payOrder(request, fromScheme:"lisheAlipayMethod",callback:nil)
    }
    ...
    func login(_ sender: UIViewController,withInfo:String,loginSuccess: @escaping (_ str:String) -> Void,loginFail:@escaping () -> Void){
        ...
        AlipaySDK.defaultService().auth_V2(withInfo:withInfo, fromScheme:"lisheAlipayMethod", callback:nil)
    }
    ...
    func showAuth_V2Result(result:NSDictionary){
        ...
        UIAlertController.showAlert(title: "授权结果", messgae: returnMsg) { (alert) in
            if returnCode == "9000" {
...
//        UIAlertController.showAlert(title: "支付结果", messgae: returnMsg) { (alert) in
//            if returnCode == "9000" {
