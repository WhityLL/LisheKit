...
//    }
    ...
    /** 根据扫描器类型配置支持编码格式 */
    static func zl_metadataObjectTypes(type: ZLScannerType) -> [AVMetadataObject.ObjectType] {
        switch type {
        case .qr:
            return [.qr]
        case .bar:
    ...
    /** 根据扫描器类型配置导航栏标题 */
    static func zl_navigationItemTitle(type: ZLScannerType) -> String {
        switch type {
        case .qr:
            return "二维码"
        case .bar:
    ...
    /** 手电筒开关 */
    static func zl_flashlight(on: Bool) {
