...
class ZLScannerView: UIView {
    ...
    init(frame: CGRect, config: ZLQRCodeCompat) {
    ...
    @objc private func flashlightClicked(button: UIButton) {
        ...
        zl_setFlashlight(on: button.isSelected)
    }
...
// MARK: - 扫描线条动画
extension ZLScannerView {
    
    /** 添加扫描线条动画 */
    func zl_addScannerLineAnimation() {
        // 若已添加动画，则先移除动画再添加
    ...
    /** 暂停扫描器动画 */
    func zl_pauseScannerLineAnimation() {
        // 取出当前时间，转成动画暂停的时间
...
extension ZLScannerView {
    ...
    /** 显示手电筒 */
    func zl_showFlashlight(animated: Bool) {
        if animated {
    ...
    /** 隐藏手电筒 */
    func zl_hideFlashlight(animated: Bool) {
...
extension ZLScannerView {
    ...
    /** 移除指示器 */
    func zl_removeActivityIndicator() {
        if self.activityIndicator != nil {
...
extension ZLScannerView {
    ...
    /** 设置手电筒开关 */
    func zl_setFlashlight(on: Bool) {
        ZLQRCodeHelper.zl_flashlight(on: on)
